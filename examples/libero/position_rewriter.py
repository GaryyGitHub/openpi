...
def _parse_regions(bddl_content: str) -> tuple[int, int, str] | None:
...
def _apply_position_perturbation(region_text: str, perturbation_type: str, seed: str) -> str:
    ...
    ranges_str = " ".join(
        f"({_fmt_coord(r[0])} {_fmt_coord(r[1])} {_fmt_coord(r[2])} {_fmt_coord(r[3])})" for r in perturbed_ranges
    )
...
def rewrite_position_variants(bddl_content: str) -> dict[str, str]:
    ...
    """
    return {"shift": rewrite_position_instruction(bddl_content, "shift")}
